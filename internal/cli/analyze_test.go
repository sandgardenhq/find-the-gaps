...
func TestAnalyze_screenshotCheck_exercisesPath(t *testing.T) {
	...
	if err := idx.RecordAnalysis(docsURL, "Covers doc page.", []string{"feature-one"}, true, "reference"); err != nil {
		t.Fatal(err)
	}
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			"choices": []map[string]any{
				{"message": map[string]any{"content": `{"gaps":[],"suppressed_by_image":[],"suppressed_by_code_block":[]}`}},
			},
		})
	}))
	defer srv.Close()
...
func TestAnalyze_anthropicProvider_usesAnthropicTokenCounter(t *testing.T) {
	...
	if err := idx.RecordAnalysis(docsURL, "A product.", []string{"feature-one"}, true, "reference"); err != nil {
		t.Fatal(err)
	}
