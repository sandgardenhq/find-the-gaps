...
- **External Runtime Dependencies** (must be on `$PATH`; install with `brew install sandgardenhq/tap/mdfetch hugo` — or `npm install -g @sandgarden/mdfetch@latest` + `brew install hugo` for go-install users):
  - `mdfetch` — our own utility that downloads websites as markdown; used to ingest the docs site
  - `hugo` — static site generator used to render the analyze report

### Distribution

- **GitHub Releases** (cross-platform binaries via goreleaser)
- **Homebrew tap** (`sandgardenhq/homebrew-tap`, exposed as `sandgardenhq/tap`). The find-the-gaps formula MUST declare:
  ```ruby
  depends_on "hugo"
  depends_on "sandgardenhq/tap/mdfetch"
  ```
  `mdfetch` is a separate formula in the same tap (sourced from `.github/homebrew/mdfetch.rb`) that wraps the `@sandgarden/mdfetch` npm package; the release workflow publishes both formulae on every tag. Configure the deps in `.goreleaser.yaml` under `brews[].dependencies`.
...
Examples: `feat/analyze-subcommand`, `fix/mdfetch-timeout-on-large-repos`, `chore/setup-project`.
