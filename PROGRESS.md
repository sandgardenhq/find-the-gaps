...
- Summary: Added a new `internal/site` package that emits a Hextra-themed Hugo static site on every `analyze` run. Exports `Build(ctx, Inputs, BuildOptions)`. Two content shapes via `--site-mode={mirror,expanded}`: mirror reproduces the markdown reports as Hextra pages (mapping/, gaps/, screenshots/); expanded adds a feature index plus per-feature pages with embedded drift findings. Three CLI flags wire it up: `--site-mode` (default `mirror`), `--no-site` (skip the build), `--keep-site-source` (preserve the generated Hugo source at `<projectDir>/site-src/` for inspection or external `hugo serve`). Hugo joins `mdfetch` as a runtime dependency: `internal/doctor` reports its version; `install-deps` installs it on macOS via `brew install hugo` and prints a download link otherwise. Hextra theme is vendored at `internal/site/assets/theme/hextra/` (v0.12.2) and embedded via `//go:embed all:assets/theme/hextra` (`all:` prefix is load-bearing — both leading-underscore Hugo theme dirs like `_partials/` and dotfiles are otherwise excluded). Templates live under `internal/site/assets/templates/` and are embedded via `//go:embed all:assets/templates`.
- Phase-by-phase:
  - **Phase 0 (Vendoring)**: Hextra v0.12.2 vendored via `make vendor-hextra`. The Make target deletes nested `go.mod` files inside the theme — without that, `go build` reports "in different module" because Hextra ships its own modules under `compose/`. Recorded VERSION pinning at `internal/site/assets/theme/hextra/VERSION`.
  - **Phase 1 (Slugs)**: `featureSlug` lowercases + NFC-normalizes via `golang.org/x/text/unicode/norm` and replaces non-`[a-z0-9]` runs with `-`. `resolveSlugs` resolves collisions deterministically by sorting feature names and appending `-2`, `-3`, … to the second-and-later occurrences.
  - **Phase 2 (Public types)**: `Mode`, `BuildOptions`, `Inputs`. Build() validates Mode + ProjectDir up front before any disk I/O.
  - **Phase 3 (Templates)**: Six Go-template files (`hugo.toml.tmpl`, `home.md.tmpl`, `feature.md.tmpl`, `features_index.md.tmpl`, `screenshot_page.md.tmpl`, plus per-section pages). Each render path uses a `*Data → *View` translation that converts `Mode` to a derived `Expanded` boolean — templates branch on `{{ if .Expanded }}`, never on a raw iota integer (which would silently break if Mode constants are reordered).
  - **Phase 4 (Materialization)**: `materialize()` dispatches by mode; `materializeMirror` writes the four section pages, copying `gaps.md` / `mapping.md` / `screenshots.md` from the project dir; `materializeExpanded` additionally renders `content/features/_index.md` (feature table) and one `content/features/<slug>/_index.md` per feature with its drift findings inlined. `linkFeatureNames` rewrites quoted feature names in the embedded gaps page into `[Name](/features/<slug>/)` markdown links.
  - **Phase 5 (Build wiring)**: `Build()` picks a srcDir (`<projectDir>/site-src/` if KeepSource, otherwise `os.MkdirTemp`), materializes into it, looks up `hugo` on PATH (returns `ErrHugoMissing` if absent), then shells out with `--minify --quiet --baseURL /`. On hugo failure the source dir is preserved and the path is included in the error so the user can inspect.
  - **Phase 6 (CLI)**: Flags added to `analyze`. `site.Build()` is called after `reporter.WriteScreenshots()` so the markdown reports always exist on disk for `materializeMirror` to read. The `ErrHugoMissing` branch returns a user-friendly stderr message naming `find-the-gaps install-deps` and `brew install hugo`.
  - **Phase 7 (Dependency declaration)**: Tasks 7.1 (`doctor` reports hugo), 7.2 (`install-deps` installs hugo), 7.4 (`.goreleaser.yaml` brews block with mdfetch + hugo dependencies and caveats), and 7.5 (README "What this installs") all landed. Task 7.3 (first-run banner) was **deferred** because no first-run banner exists in the source — `FIND_THE_GAPS_QUIET` is referenced only in CLAUDE.md and `.plans/`, never implemented. Building the banner subsystem (config-dir state tracking, env-var gate, tests) is its own feature; deferred with rationale in the task list. Task 7.4 created `.goreleaser.yaml` from scratch (the file did not exist before this branch). It is dormant alongside the active `.github/workflows/release.yml` (a hand-rolled `go build` + `gh release create` pipeline that does NOT use goreleaser); the config makes the Homebrew dependency declaration concrete now rather than at migration time.
  - **Phase 8 (testscript end-to-end)**: Task 8.6 (`doctor_hugo.txtar`) landed. Tasks 8.1–8.5 were **deferred** because they all require an LLM/spider fake harness that does not exist in source — `analyze` short-circuits at `analyze.go:123` when `--docs-url` is empty, so a testscript cannot reach `site.Build()` without a fake harness. The plan referenced `FTG_LLM_FAKE=1` but that env var is documented only in the plan, not implemented. The same code paths are exercised end-to-end by the `internal/site` integration tests (Tasks 5.1, 5.2 — they call `Build()` directly with a real hugo binary).
  - **Phase 9 (Verification & Cleanup)**: Scenario 10 added to `.plans/VERIFICATION_PLAN.md` covering all flag combinations and the missing-hugo error path. Final coverage + lint sweep done.
- Tests: `go test ./...` green; `go test -tags integration ./internal/site/` green. New testscript `cmd/find-the-gaps/testdata/script/doctor_hugo.txtar` passes.
- Coverage: `internal/site` 85.4% of statements with the integration build tag (matches the figure recorded at Task 5.5). The remaining 14.6% is dominated by filesystem-error wrappers (`return fmt.Errorf("...: %w", err)` after `os.WriteFile` / `os.MkdirAll` / `os.RemoveAll`) and template-execute error branches in the renderer functions. Both classes of branches are internal-trust boundaries that CLAUDE.md says not to test (no value beyond the wrapped error). The four meaningful Build error paths (unknown mode, empty ProjectDir, hugo missing, hugo non-zero exit) are each covered by a dedicated test in `internal/site/site_test.go`. Decision: accept 85.4% with rationale rather than chase coverage by mocking the filesystem.
- Build: OK (`go build ./...`).
- Linting: OK (`golangci-lint run` — 0 issues across the repo).
- Notable fixes recorded along the way:
  - `feature.md.tmpl` initially used `{{- $status :=` which trimmed the newline after `title = "..."` and produced invalid TOML. Fixed by removing the `-` and locked in by `TestRenderFeatureFrontmatterIsValidTOML` (commit `b728a10`).
  - `//go:embed assets/theme/hextra` skipped Hextra's `_partials/` directory because Go's embed defaults exclude leading-underscore dirs. Fixed by adding the `all:` prefix (commit `0b75c12`).
  - Hextra ships nested `go.mod` files under `compose/` that would otherwise cause "in different module" build errors; the `vendor-hextra` Makefile target now deletes them after rsync (commit `95afd8f`).
- Commits on branch `feat/hugo-site-output`: `c3570f0` vendor theme, `afc64f9` slug helper, `717dc94` slug collision resolver, `fdfceb2` types + Build stub, `e0d6bb6` embed asset trees, `95afd8f` strip nested go.mod, `9b0e55e` hugo.toml template, `ae787c7` home template, `96aca88` feature template, `351fc01` features-index template, `73f426c` screenshot template, `5f0db01` materialize mirror, `fadd47f` materialize with screenshots, `ab07fcb` materialize expanded, `0b75c12` embed `all:` fix, `b3b4314` Build mirror happy path, `b728a10` TOML newline fix, `735b69f` keep-source + expanded integration, `b51474d` ErrHugoMissing test, `c8eb805` hugo-failure preserve-source test, `cd9fb45` coverage sweep to 85.4%, `0a4ea93` CLI flags, `a198001` wire site.Build into analyze, `116e7e2` doctor reports hugo, `6333ab4` install-deps installs hugo, `62ada9f` README lists hugo, `a33acae` doctor_hugo testscript, `e64fc9d` Scenario 10, `a9a61cc` PROGRESS update, `267be3d` goreleaser config.
- Completed: 2026-04-27
