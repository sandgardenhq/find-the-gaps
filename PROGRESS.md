...
- Phase-by-phase:
  ...
  - **Phase 7 (Dependency declaration)**: Tasks 7.1 (`doctor` reports hugo) and 7.2 (`install-deps` installs hugo) both landed. Tasks 7.3 (first-run banner), 7.4 (goreleaser Homebrew dep), and 7.5 (README) were addressed as follows: 7.3 and 7.4 were **deferred** because the prerequisite infrastructure (a first-run banner subsystem; a `.goreleaser.yaml` config) does not exist in the repo — the original plan presumed both already existed. Both are recorded in the task list with deferral rationale, to be picked up when those subsystems are introduced. 7.5 (README "What this installs" lists hugo) landed.
...
- Notes: Three deferrals (Tasks 7.3, 7.4, 8.1–8.5) all share the same shape — the plan presumed prerequisite infrastructure that does not exist in the repo (a first-run banner; a goreleaser config; a fake-LLM harness for testscripts). Building any of those is its own feature. The Hugo functionality itself is complete and exercised end-to-end by the `internal/site` integration tests; the deferrals only affect the auxiliary surfaces (release tooling, banner copy, testscript harness). Branch is ready for PR.
